...
import {
  parseReportHubPreferences,
  setReportHubPreferences,
  type ReportHubPreferences,
...
function normalizePayload(body: unknown): ReportHubPreferences {
  const raw = (body as { reportHub?: unknown; sectionOrder?: unknown } | null)?.reportHub ?? body;
  const parsed = parseReportHubPreferences({ reportHub: raw });
...
export async function GET() {
  ...
  const reportHub = parseReportHubPreferences(user?.uiPreferences) ?? { sectionOrder: {} };
  return NextResponse.json({ reportHub }, { status: 200 });
}
...
export async function PATCH(req: Request) {
  ...
  const reportHub = normalizePayload(body);
  ...
  await getPrismaUser().update({
    where: { id: auth.userId },
    data: { uiPreferences: setReportHubPreferences(existing?.uiPreferences, reportHub) },
  });
