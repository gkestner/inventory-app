...
function itemSearchHaystack(it: ItemLite): string {
  // Put everything searchable into one string.
  return normalize(
    [
      it.sku,
      it.partNumber ?? "",
...
export default function ItemPicker({
  ...
  const filtered = useMemo(() => {
    ...
    // If user is showing the selected label, don’t filter aggressively until they type.
    // But if dropdown is open and query is empty, show top N items.
    if (toks.length === 0) return items.slice(0, 80);

    const out: ItemLite[] = [];
    for (const it of items) {
      const hay = itemSearchHaystack(it);
      let ok = true;
      for (const t of toks) {
        if (!hay.includes(t)) {
          ok = false;
  ...
  function selectItem(it: ItemLite) {
    ...
    setOpen(false);
    // keep focus on input for fast workflow
  ...
  return (
    <div
      ...
      style={{
        position: "relative",
        isolation: "isolate", // ✅ makes z-index behave predictably
        width: "100%",
        minWidth: 0,
        ...style,
      }}
    >
      ...
      <input
        ...
        onChange={(e) => {
          ...
          setOpen(true);
          setActiveIndex(0);
          // If they start typing, treat as “not selected yet” until they pick
          setSelectedId("");
        }}
      ...
      {open ? (
        <div
          style={{
            ...
            right: 0,
            top: "calc(100% + 8px)",
            zIndex: 99999, // ✅ sit above everything
            border,
            borderRadius: 12,
            background: surface, // ✅ opaque background prevents “mixing”
              ...
              filtered.map((it, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {sub ? (
                      <div style={{ fontSize: 12, opacity: 0.75, marginTop: 2, lineHeight: 1.25 }}>{sub}</div>
                    ) : null}
                  </button>
                );
              })
