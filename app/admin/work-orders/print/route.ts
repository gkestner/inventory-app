...
export async function GET(req: Request) {
  ...
  const userGroups = Array.from(
    orderedRows.reduce(
      (groups, row) => {
        const userKey = `${row.createdByUser?.id ?? "none"}:${row.createdByUser?.email ?? ""}`;
        const userLabel = row.createdByUser ? `${row.createdByUser.name} (${row.createdByUser.email})` : "-";
        ...
        groups.set(userKey, { userLabel, rows: [row] });
        return groups;
      },
      new Map<string, { userLabel: string; rows: Array<(typeof orderedRows)[number]> }>()
    )
  );
  ...
  <div class="cover-meta">Work orders in batch: <b>${group.rows.length}</b></div>
      ...
      const cardHtml = group.rows
        .map((r, index) => {
          ...
          const cardClass = index === group.rows.length - 1 ? "card keep last-card-in-batch" : "card keep";
  ...
  <div class="head">
    ...
    <div class="meta right">Created: ${escapeHtml(fmtLocal(r.createdAt))}</div>
  </div>
  ...
  ${r.generatedAt ? `<div class="meta right" style="margin-top:10px;">Generated by Office: <b>${escapeHtml(fmtLocal(r.generatedAt))}</b></div>` : ""}
</section>`;
        })
        .join("\n");
...
<head>
  ...
  <style>
    ...
    .last-card-in-batch { break-after: auto; page-break-after: auto; }
